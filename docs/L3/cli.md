...
- **`@koi/tui` — `PermissionPrompt` width fix (#1913)**: modal now has an explicit width so OpenTUI's `blendCells` doesn't busy-loop. No CLI surface change — `terminalWidth` is forwarded from `useTerminalDimensions()` already wired in the TUI host.
