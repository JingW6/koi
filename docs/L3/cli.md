...
- **`@koi/tui` — `PermissionPrompt` width fix (#1913)**: modal now has an explicit width so OpenTUI's `blendCells` doesn't busy-loop. `terminalWidth` and `terminalHeight` are forwarded from `useTerminalDimensions()`. Height guard `computeMinSafeHeight` dynamically budgets the full prompt row count and suppresses approval when the terminal is too short.
